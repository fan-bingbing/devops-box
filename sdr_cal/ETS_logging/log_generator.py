...
class Logging:
    def __init__(self, log_path, run_test):
        self.passed = True
    ...
    def create_log_directory_top_level(self):
        ...
        while True:
            try:
                top_level_dir_path = self.log_path + '/' + date_time + '_' + self.run_test + index
    ...
    def create_test_results_path(self):
        date_time = datetime.now().strftime("%Y_%m_%d_%H%Mh")
        index = ''
        while True:
            try:
                current_test_path = self.top_level_dir_path + date_time + index
    ...
    def save_log(self):
        date_time = datetime.now().strftime("%Y_%m_%d_%H%Mh")
        pd.DataFrame(self.log_dict).to_csv(self.top_level_dir_path + '/Test_Results_'  + date_time + '.csv', index=False)
        self.log_dict = None

    def save_test_overview_log(self):
        date_time = datetime.now().strftime("%Y_%m_%d_%H%Mh")
        pd.DataFrame(self.test_overview_log).to_csv(self.top_level_dir_path + '/' + 'Test_Result_Overview_' + self.run_test + '_' + date_time + '.csv', index=False)

    def test_setup_log(self, equip_config):

        date_time = datetime.now().strftime("%Y_%m_%d_%H%Mh")

        with open(self.top_level_dir_path + '/' + 'Test_Setup_Log_' + self.run_test + '.txt', 'a') as f:

            print('Date/Time: ', date_time + '\n', file=f)
            print('Test: ', self.run_test + '\n', file=f)
